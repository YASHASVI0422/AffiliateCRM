    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={scoreAll} disabled={scoringAll} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all disabled:opacity-50" style={{background:'rgba(139,92,246,0.12)', border:'1px solid rgba(139,92,246,0.2)', color:'#c4b5fd'}}>{scoringAll?<div className="w-4 h-4 border-2 border-violet-400 border-t-transparent rounded-full animate-spin"/>:<Brain size={14}/>}AI Score All</button>
          {isAdmin && (
                ...
                return (
                  <tr key={l._id} className="table-row">
                    ...
                    <td className="px-4 py-3">
                      ...
                      :<button onClick={()=>scoreSingle(l._id)} disabled={scoringId===l._id} className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg transition-all disabled:opacity-50" style={{background:'rgba(139,92,246,0.08)', border:'1px solid rgba(139,92,246,0.15)', color:'#c4b5fd'}}>{scoringId===l._id?<div className="w-3 h-3 border-2 border-violet-400 border-t-transparent rounded-full animate-spin"/>:<Sparkles size={11}/>}Score</button>}
                    </td>
                    <td className="px-4 py-3"><div className="flex gap-1">
                      <button onClick={()=>genNotes(l._id)} title="AI Notes" className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-violet-400 hover:bg-violet-500/10 transition-all"><FileText size={14}/></button>
                      <button onClick={()=>openEdit(l)} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-cyan-400 hover:bg-cyan-500/10 transition-all"><Edit2 size={14}/></button>
      ...
      <Modal isOpen={noteModal} onClose={()=>setNoteModal(false)} title="AI Generated Notes" size="md">
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-xs rounded-xl px-3 py-2" style={{background:'rgba(139,92,246,0.08)', border:'1px solid rgba(139,92,246,0.15)', color:'#c4b5fd'}}><Brain size={13}/>Generated by Claude AI</div>
          {genNote?<div className="flex items-center justify-center gap-3 py-6"><div className="w-5 h-5 border-2 border-violet-400 border-t-transparent rounded-full animate-spin"/><span className="text-slate-500 text-sm">Writing notes...</span></div>
